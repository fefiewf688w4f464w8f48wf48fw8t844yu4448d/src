...
namespace MEM_LIB {
	...
	uintptr_t getBaseAddr(DWORD processId, const wchar_t* ModuleTarget) {
	...
	DWORD procId = GetProc(L"cs2.exe");
	uintptr_t baseAddr = getBaseAddr(procId, L"client.dll");
	...
	type memRead(uintptr_t pointerStatic) {
		type value = { };
		ReadProcessMemory(MEM_LIB::procHandle, (LPVOID)pointerStatic, &value, sizeof(type), NULL);
		return value;
	}

	template <typename type>
	bool memWrite(uintptr_t pointerStatic, type value) {
		return WriteProcessMemory(MEM_LIB::procHandle, (LPVOID)pointerStatic, &value, sizeof(type), NULL);
	}
