...
//kresliace funkcie
void renderSquareBorder(HDC hdc, int x, int y, int w, int h, COLORREF borderColor) {
...
void renderAllPlayers(HDC hdc) {
    for (int i = 0; i < 64; i++) {
        ...
        const auto local_player_pawn = MEM_LIB::memRead<std::uintptr_t>(MEM_LIB::baseAddr + Adresy::dwLocalPlayerPawn);

        //ciselne udaje mam zo pseudokodu zo suboru client.dll(pouzil som na to IDA Pro)
        const auto Entity = MEM_LIB::memRead<std::uintptr_t>(MEM_LIB::baseAddr + Adresy::dwEntityList);
        if (Entity == 0)
            continue;

        const auto listEntity = MEM_LIB::memRead<std::uintptr_t>(Entity + ((8 * (i & 0x7FFF) >> 9) + 16));
        if (listEntity == 0)
            continue;

        const auto entityController = MEM_LIB::memRead<std::uintptr_t>(listEntity + (120) * (i & 0x1FF));
        if (entityController == 0)
            continue;

        const auto entityControllerPawn = MEM_LIB::memRead<std::uintptr_t>(entityController + Adresy::m_hPlayerPawn);
        if (entityControllerPawn == 0)
            continue;

        const auto listEntity2 = MEM_LIB::memRead<std::uintptr_t>(Entity + (0x8 * ((entityControllerPawn & 0x7FFF) >> 9) + 16));
        if (listEntity2 == 0)
            continue;

        const auto entityPawn = MEM_LIB::memRead<std::uintptr_t>(listEntity2 + (120) * (entityControllerPawn & 0x1FF));

        if (entityPawn == 0)
            continue;

        //3D suradnice hraca
        const auto oldOrigin = MEM_LIB::memRead<Vector3>(entityPawn + Adresy::m_vOldOrigin);
        ...
        const auto viewMatrix = MEM_LIB::memRead<view_matrix_t>(MEM_LIB::baseAddr + Adresy::dwViewMatrix);
        ...
        const auto entityPLRHealth = MEM_LIB::memRead<int>(entityPawn + Adresy::m_iHealth);
        const auto entityPLRTeam = MEM_LIB::memRead<int>(entityPawn + Adresy::m_iTeamNum);

        const auto localPLRTeam = MEM_LIB::memRead<int>(local_player_pawn + Adresy::m_iTeamNum);
            ...
            if (entityPawn != local_player_pawn) {
                ...
                if (entityPLRHealth != 0) {
                    ...
                    if (localPLRTeam == entityPLRTeam) {
                        ...
                        renderSquareBorder(hdc, screenhead.x - width / 2, screenhead.y,
                        ...
                        DrawLine(hdc, GetSystemMetrics(SM_CXSCREEN) / 2,
                            ...
                            screenhead.x - width / 7.75, screenhead.y,
                            RGB(255, 255, 255));
                    }
                    else {
                        //deje sa tu to iste ale farba je ina ak hrac sa nenachadza v mojom teame
                        renderSquareBorder(hdc, screenhead.x - width / 2, screenhead.y, width, height, RGB(255, 0, 0));
                        DrawLine(hdc, GetSystemMetrics(SM_CXSCREEN) / 2,
                            GetSystemMetrics(SM_CYSCREEN) / 6,
                            screenhead.x - width / 7.75, screenhead.y,
                            RGB(255, 0, 0));
...
int main()
{
    ...
    HWND hwnd = CreateWindowEx(WS_EX_LAYERED | WS_EX_TOPMOST | WS_EX_TRANSPARENT, L"GDIOverlay",L"GDI_Overlay", WS_POPUP, 0, 0, screenWidth, screenHeight, NULL,NULL, GetModuleHandle(NULL), NULL);
